...
${basename(__FILE__, ".php")} = function () {
    if ($this->get_request_method() == "POST") {
        ...
        if ($request_url == "/api/mail/otp") {
            if (isset($this->_request['username']) and isset($this->_request['subject']) and isset($this->_request['message']) and isset($this->_request['torecieve']) and isset($this->_request['org_name']) and isset($this->_request['otp'])) {
                ...
                $mailClass->HandleOTPRequest();
            } else {
                $data = [
                    "Status" => "Bad Request",
                    "Status Code" => 400,
                    "Error" => "Make sure Username, useremail, subject, message, torecieve are properly filled to send the request",
                ];
